...
function PredictionContent(props){
  ...
  // (UP - CONFIRMED, DOWN, SEARCHING) and a tighter size for longer multi-word titles
    ...
    <div className="flex flex-col flex-1 gap-3">
      <div className="flex flex-col items-center text-center pt-1">
        ...
        {/* V143: Smart prediction text routing — split long status messages into a tight
                  ...
                  (UP - CONFIRMED, DOWN - CONFIRMED, UP (FORMING), SEARCHING...) render as the big heading.
        ...
        {(()=>{
          ...
          const isShort=p.includes('CONFIRMED')||p.includes('FORMING')||p==='SEARCHING...'||p==='NO CALL'||p==='CLOSED'||p.includes('WAITING FOR MOMENTUM')||p.includes('SIGNAL CLEARED');
          if(isShort){
      ...
      <div className={'flex flex-col gap-1.5 border-t border-[#E8E9E4]/10 pt-3 mt-4'}>
...
function TaraCallCard({taraCall,taraScorecards,taraCallLog,windowType,timeState,analysis,className,taraLearnings,onSoftHint,onHardForce,kalshiYesPrice}){
  if(!taraCall)return null;
    ...
    // V6.0.4: Surface live gate info during scanning. Previously this hid all developing
    //   info behind generic "Scanning for direction" — user couldn't tell which way she
    //   was leaning or what was blocking commit, so by the time she locked, Kalshi had
    //   already priced in the move. Now we show the specific gate reason (e.g. "Coin-flip
    //   — posterior 53% only 3pt off neutral") so the user has actionable info early.
    const dispReason=snap
      ?(snap.reason||(isSatOutSnap?'Sat out — no edge this window':'Locked'))
      :(tc?.reason||'Scanning for direction.');
    // V6.0.4: live lean — gate sees a direction but hasn't committed yet
    // V6.0.5: lean threshold 4→5pt. Real lean = ≥5pt off neutral. Below that, headline
    //   stays SCANNING. The lean info still surfaces in the Decision Clock + reason.
    const _liveLeanDir=!snap&&tc?.direction&&tc?.conviction>=5?tc.direction:null;
    const _liveLeanConv=tc?.conviction||0;
    // V6.0.5: 5-state model. Was 4-state (SCANNING / LOCKED UP / LOCKED DOWN / SITTING OUT).
    //   Now adds WATCHING UP / WATCHING DOWN as headline states when conv ≥ 5pt and no snap.
    //   User feedback: needed direction visibility BEFORE commit so they can enter Kalshi
    //   while odds are still actionable. WATCHING uses muted-color version of LOCKED hues
    //   so it reads as "leaning" not "committed."
    ...
    const phaseLabel=snap?(isSatOutSnap?'SITTING OUT':'LOCKED'):isWatching?'WATCHING':'SCANNING';
    ...
    // V6.0.5: Decision deadline. Tara's endgame freeze fires when remaining < 90s (15m)
    ...
    if(snap){
      ...
      // V6.0.5: ALWAYS show a real time number in the countdown. Three cases:
      ...
      if(_etaFitsCountdown){
        ...
        phaseHint=`then ${_deadlineLabel} of scan window`;
        phaseProgressPct=Math.min(15,_elapsed*0.7);
      } else {
        countdownText=`${_deadlineLabel} to decide`;
        phaseHint=tc?.reason?tc.reason.slice(0,60):'no clear direction yet';
    ...
    return(
      <div className={"mb-3 px-3 py-3 rounded-lg shrink-0 "+(className||"")} style={{background:bgClr,border:'1px solid '+borderClr,boxShadow:isLockedSnap?`inset 0 0 24px ${snap.call==='UP'?'rgba(52,211,153,0.06)':'rgba(244,114,182,0.06)'}`:'none'}}>
        ...
        {!snap&&(()=>{
          // V6.0.6: cap lock ETA to deadline. The rate-based ETA can mathematically exceed
          //   remaining window time when samples are accumulating very slowly (e.g. signal
    ...
    return React.createElement('div',{className:'mb-4'},
      ...
      React.createElement('div',{className:'space-y-1'},
        entries.map(([k,v])=>{
          const total=v.wins+v.losses;
          const wr=Math.round((v.wins/total)*100);
          const adj=data.multipliers?.confluenceConfBoost?.[k]||0;
...
function TaraMemoryModal({taraCallLog,onClose}){
  ...
  const _periodFromWindowId=(wid,wt)=>{
    ...
    try{
      ...
      const _f=(d)=>`${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;
...
function StatsView({tradeLog,scorecards,taraCallLog,onClose}){
  ...
  const allResolved=React.useMemo(()=>{
    ...
    const taraCalls=(taraCallLog||[]).filter(e=>e&&(e.result==='WIN'||e.result==='LOSS')&&(e.dir==='UP'||e.dir==='DOWN')).map(e=>({
      ...
      regime:e.regime,
      direction:e.dir,
      dir:e.dir, // alias for drill row template
      session:e.session,
      hour:new Date(e.id||e.time||0).getHours(),
      windowType:e.windowType,
      windowId:e.windowId, // V5.7.5: for period derivation
      strike:e.strike,
      closingPrice:e.closingPrice,
      closeSource:e.closeSource,
      posterior:e.posterior, // V5.7.5: drill row reads this
      qScore:e.qScore,
      qualityScore:e.qScore, // alias for drill row template
      fgt:e.fgt,
      fgtAlignment:e.fgt, // alias for drill row template
      closingGapBps:e.gapBps, // alias for drill row template
      tier:e.tier,
      source:'tara',
    }));
            ...
            <div className="mt-4 pt-4" style={{borderTop:'1px solid '+T2_GOLD_GLOW}}>
              ...
              <div className="space-y-1">
                {drillTrades.slice(0,12).map((t,i)=>{
                  ...
                  // V5.7.5: Window period derived from windowId (encodes bucket-start ISO).
                  let _period=null;
                  if(t.windowId){
                    try{
                      const m=t.windowId.match(/-(.+)$/);
                      if(m){
                        const start=new Date(m[1]);
                        if(!isNaN(start.getTime())){
                          const winMs=t.windowType==='15m'?900000:300000;
                          const end=new Date(start.getTime()+winMs);
                          const _f=(d)=>`${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;
                          _period=`${_f(start)}–${_f(end)}`;
                        }
                      }
                    }catch(e){}
                  }
                  ...
                  return(
                    <div key={i} className="px-2 py-1.5 rounded text-[11px] hover:bg-[#0E100F]/40">
                      ...
                      <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        {_period&&<span className="text-[#E8E9E4]/55 shrink-0 hidden sm:inline" style={T2_MONO_STYLE}>{_period}</span>}
                        ...
                        <span className="text-[#E8E9E4]/40 w-12 hidden sm:block" style={T2_MONO_STYLE}>{t.posterior?t.posterior.toFixed(0)+'%':'—'}</span>
                      ...
                      {_strikeFmt&&_closeFmt&&(
                        <div className="flex items-center gap-2 mt-0.5 pl-[72px] text-[10px] text-[#E8E9E4]/40">
                          <span style={T2_MONO_STYLE}>Strike {_strikeFmt} → Close {_closeFmt}</span>
                          {t.closeSource==='kalshi'&&<span className="italic text-[#E8E9E4]/30 hidden md:inline">· kalshi-settled</span>}
                        </div>
                      )}
                    </div>
                  );
...
function TapeStrip({tapeWindows,whaleLog}){
  ...
  const trendLabel=_trend==='rising'?(dominant==='BUY'?'↑ accelerating buy':'↑ buy reversing'):
                  ...
                  null;
  // V6.3.2: TAPE QUALITY — 3-light system summarizing whether the tape signal is trustworthy.
  //   Three checks: (A) do 30s + 60s windows agree directionally and at meaningful strength?
  //   (B) is dollar volume in 60s above noise-floor (>$500K)? (C) do recent whales align with
  //   the tape direction? Each check returns true/false. Output: GREEN if all 3, AMBER if 2,
  //   ROSE if 0-1, GREY if tape is too thin to even score.
  const _quality=(()=>{
    const _w30Pct=w30.buyPct;
    const _w60Pct=w60.buyPct;
    const _w30Tot=(w30.buys||0)+(w30.sells||0);
    const _w60Tot=(w60.buys||0)+(w60.sells||0);
    if(_w30Tot<TAPE_FLOORS.w30||_w60Tot<TAPE_FLOORS.w60||_w30Pct==null||_w60Pct==null){
      return{level:'thin',windowsAgree:null,volMeaningful:null,whalesAlign:null,dirText:'TAPE THIN'};
    }
    // (A) Windows agree: same side AND each ≥60% (real strength, not coin-flip)
    const _w30Side=_w30Pct>=50?'BUY':'SELL';
    const _w60Side=_w60Pct>=50?'BUY':'SELL';
    const _w30Strength=Math.max(_w30Pct,100-_w30Pct);
    const _w60Strength=Math.max(_w60Pct,100-_w60Pct);
    const windowsAgree=_w30Side===_w60Side&&_w30Strength>=60&&_w60Strength>=60;
    // (B) Volume meaningful: 60s window has >$500K total
    const volMeaningful=_w60Tot>=500000;
    // (C) Whales align: among whales in last 60 seconds, ≥60% match dominant tape direction
    const _dominantSide=_w60Side; // use 60s as "dominant"
    let whalesAlign=null;
    if(Array.isArray(whaleLog)&&whaleLog.length>0){
      const _now=Date.now();
      const _recentWhales=whaleLog.filter(w=>w&&w.time&&(_now-w.time)<60000);
      if(_recentWhales.length>=2){
        // Whale side comes from the trade type — 'B' = buy, 'S' = sell
        const _whaleBuyCount=_recentWhales.filter(w=>w.side==='B'||w.side==='BUY'||w.t==='B').length;
        const _whaleAlignedCount=_dominantSide==='BUY'?_whaleBuyCount:(_recentWhales.length-_whaleBuyCount);
        whalesAlign=_whaleAlignedCount>=Math.ceil(_recentWhales.length*0.6);
      }
    }
    // Score: count true checks (treat null whale as neutral, doesn't add or subtract)
    let _score=0;
    if(windowsAgree)_score+=1;
    if(volMeaningful)_score+=1;
    if(whalesAlign===true)_score+=1;
    else if(whalesAlign===false)_score-=0.5; // slight penalty for divergent whales
    const level=_score>=2.5?'high':_score>=1.5?'medium':'low';
    return{level,windowsAgree,volMeaningful,whalesAlign,dirText:_dominantSide};
  })();
        ...
        <div className="flex items-baseline gap-2.5">
          ...
          {_quality.level!=='thin'&&(()=>{
            ...
            const _tooltip=[
              ...
              `Volume meaningful: ${_quality.volMeaningful?'yes':'no'} (60s ≥ $500K)`,
              `Whales align: ${_quality.whalesAlign===true?'yes':_quality.whalesAlign===false?'no':'n/a'} (last 60s)`,
            ].join(' · ');
            return(
              <span className="inline-flex items-center gap-1.5" title={_tooltip}>
                <span className="inline-flex items-center gap-0.5">
                  {_checks.map((c,i)=>React.createElement('span',{
                    ...
                    style:{background:c===true?_color:c===false?'#f87171':'rgba(232,233,228,0.18)'},
                  }))}
                </span>
          ...
          return(
            <div className="space-y-1">
              ...
              {(()=>{
                ...
                const _colorFor=(d)=>d==='UP'?'text-emerald-300':d==='DOWN'?'text-rose-300':'text-[#E8E9E4]/30';
                ...
                return React.createElement(React.Fragment,null,
                  ...
                  React.createElement('div',{className:'flex items-center gap-2 text-[10px] pt-1.5 mt-0.5',style:{borderTop:'1px solid rgba(196,181,253,0.18)'}},
                    ...
                    React.createElement('span',{className:'w-10 shrink-0 text-right font-mono text-[10px] text-[#E8E9E4]/40'},_structCount>=4?'gate':_structCount>=3?'edge':'mixed')
                  ),
                  // Grand Trend per-timeframe row
                  React.createElement('div',{className:'flex items-center gap-2 text-[10px]'},
                    React.createElement('span',{className:'w-16 shrink-0 text-[10px] text-[#E8E9E4]/55'},'Grand Tr.'),
                    React.createElement('div',{className:'flex-1 flex items-center gap-1.5 text-[10px]'},
                      ['1m','5m','15m'].map((tf,i)=>React.createElement('span',{key:tf,className:'flex items-baseline gap-0.5'},
                        React.createElement('span',{className:'text-[#E8E9E4]/35 text-[9px]'},tf),
                  ...
                  React.createElement('div',{className:'flex items-center gap-2 text-[10px]'},
                    React.createElement('span',{className:'w-16 shrink-0 text-[10px] text-[#E8E9E4]/55'},'Trend Ch.'),
                    React.createElement('div',{className:'flex-1 flex items-center gap-1.5 text-[10px]'},
                      ['1m','5m','15m'].map((tf,i)=>React.createElement('span',{key:tf,className:'flex items-baseline gap-0.5'},
                        React.createElement('span',{className:'text-[#E8E9E4]/35 text-[9px]'},tf),
      ...
      if(!lockedCallRef.current){
        ...
        if(isVeryLateLock&&_trajStrength<10&&_fgtAlignAbs<4){
          ...
          if(!dirAllowed){
            ...
            } else if(Date.now()-lockReleasedAtRef.current<(confluenceStateRef.current.isConfluent?15000:45000)){
              // V5.6.11: post-release cooldown — must scan for 45s before re-locking.
              // V5.7.8: shortened to 15s when current candidate is confluent — orthogonal-signal
              //   agreement is strong enough evidence to re-engage faster after a release.
              const _coolMs=confluenceStateRef.current.isConfluent?15000:45000;
        ...
        //       FGT alignment is strong enough evidence to overcome regime hesitancy.
          ...
          if(!dirAllowed){
            ...
            if(!momentumOK){
              ...
              // V5.7.8: shortened cooldown on confluence — see UP path comment.
              const _coolMs=confluenceStateRef.current.isConfluent?15000:45000;
          ...
          <div className={`bg-[#181A19] p-3 sm:p-4 rounded-xl border border-[#E8E9E4]/10 shadow-md flex flex-col relative ${mobileTab!=='signal'?'hidden lg:flex':''}`}>
            ...
            <div className="flex justify-between items-center mb-3 shrink-0">
              ...
              <button onClick={()=>{
                ...
                if(userPosition&&manuallyClosedRef.current===null&&taraAdviceRef.current.includes('CONFIRMED')){
                  const offerVal=parseFloat(currentOffer)||0;
            ...
            <div className={'p-4 sm:p-6 space-y-5 text-xs sm:text-sm text-[#E8E9E4]/80'}>
              ...
              <section className="mb-2 pb-3" style={{borderBottom:'1px solid '+T2_GOLD_GLOW}}>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-[9px] uppercase tracking-[0.18em] font-bold" style={{color:T2_GOLD}}>Tape Quality Badge</span>
                  <span className="text-[9px] uppercase tracking-wider text-[#E8E9E4]/30">2026.05.03</span>
                </div>
                <h3 className="font-serif text-2xl mb-2 tracking-tight text-white">Tara <span style={{color:T2_GOLD}}>6.3.2</span> — At-a-Glance Tape Trustworthiness</h3>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-3">User concern: &ldquo;huge money numbers but sometimes i&rsquo;m confused.&rdquo; Built a 3-light quality summary so you don&rsquo;t have to mentally average percentages.</p>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-3 mb-2">The three checks</div>
                <ol className="list-decimal pl-4 space-y-1.5 text-[11px]">
                  <li><strong>Windows agree:</strong> 30s and 60s point the same direction AND each is ≥60% strength. Real consensus, not coin-flip noise.</li>
                  <li><strong>Volume meaningful:</strong> 60s window has ≥$500K total dollar flow. Below this, percentages can be skewed by single trades.</li>
                  <li><strong>Whales align:</strong> Of whales in the last 60 seconds, ≥60% match the dominant tape direction. Confirms size + flow agreement.</li>
                </ol>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">Reading the badge</div>
                <ul className="list-disc pl-4 space-y-1 text-[11px]">
                  <li><span style={{color:'#6ee7b7'}}>● ● ●</span> <strong>STRONG</strong> — all 3 checks pass, tape is the truth</li>
                  <li><span style={{color:'#fbbf24'}}>● ● ○</span> <strong>MIXED</strong> — 2 checks pass, treat as suggestive not definitive</li>
                  <li><span style={{color:'#f87171'}}>● ○ ○</span> <strong>WEAK</strong> — 0-1 checks pass, ignore the percentages</li>
                  <li>Badge hides entirely when tape is too thin to score (no useful signal yet)</li>
                </ul>

                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mt-3">Hover the badge for tooltip showing exact check results. Whales-align reads as &ldquo;n/a&rdquo; when fewer than 2 whales in the last 60s &mdash; not enough sample size to score.</p>

                <p className="text-xs text-[#E8E9E4]/55 leading-relaxed mt-4 italic">Use it like a confidence dial on the tape numbers. STRONG + 70% sell = real DOWN signal. WEAK + 70% sell = noise, ignore.</p>
              </section>
              ...
              <section className="mb-2 pb-3" style={{borderBottom:'1px solid '+T2_GOLD_GLOW}}>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-[9px] uppercase tracking-[0.18em] font-bold" style={{color:T2_GOLD}}>Critical Crash Fix</span>
                  <span className="text-[9px] uppercase tracking-wider text-[#E8E9E4]/30">2026.05.03</span>
                </div>
                <h3 className="font-serif text-2xl mb-2 tracking-tight text-white">Tara <span style={{color:T2_GOLD}}>6.2.9</span> — TDZ Fix</h3>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-3">User reported screen showing &ldquo;ERROR&rdquo; in the prediction column with stack trace pointing to <code className="text-[10px] bg-[#0E100F] px-1">ReferenceError: Cannot access &lsquo;We&rsquo; before initialization</code> from useMemo. Real bug introduced in V6.2.8.</p>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-3 mb-2">What broke</div>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed">V6.2.8 added Tara-aware data passing to the advisor. Inside the <code className="text-[10px]">analysis</code> useMemo block, I referenced <code className="text-[10px]">taraCall.direction</code> and <code className="text-[10px]">taraCall.posterior</code>. But <code className="text-[10px]">taraCall</code> is a const that&rsquo;s computed AFTER the analysis useMemo — closure-over-let creates a temporal dead zone, throws on first render. Minified variable name shows up as &lsquo;We&rsquo;.</p>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">The fix</div>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-2">Replaced taraCall references with equivalent data from sources that ARE in scope at that point:</p>
                <ul className="list-disc pl-4 space-y-1 text-[11px]">
                  <li><strong>taraSnapshot:</strong> from <code className="text-[10px]">taraCallSnapshotRef.current</code> — refs are always safe</li>
                  <li><strong>taraLean:</strong> derived from <code className="text-[10px]">taraCallSampleRef.current.dir</code> + posterior conviction</li>
                  <li><strong>taraPosterior:</strong> just <code className="text-[10px]">posterior</code> directly (same value taraCall would carry)</li>
                </ul>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mt-2">Same V6.2.8 advisor behavior — minimal mode when no position, full cross-reference when manually positioned, &ldquo;would have leaned&rdquo; on SIT_OUT — just no longer crashes.</p>
              </section>
              ...
              <section className="mb-2 pb-3" style={{borderBottom:'1px solid '+T2_GOLD_GLOW}}>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-[9px] uppercase tracking-[0.18em] font-bold" style={{color:T2_GOLD}}>Tara-Only Trade Log</span>
                  <span className="text-[9px] uppercase tracking-wider text-[#E8E9E4]/30">2026.05.03</span>
                </div>
                <h3 className="font-serif text-2xl mb-2 tracking-tight text-white">Tara <span style={{color:T2_GOLD}}>6.2.5</span> — Only Tara&rsquo;s Calls Get Logged</h3>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-2">User: &ldquo;stop completely saving trades from general prediction. just keep saving my score. i only want trades taken by tara to be logged.&rdquo; Reverses V6.2.3.</p>
                <ul className="list-disc pl-4 space-y-1 text-[11px]">
                  <li><strong>General predictor per-trade detail log:</strong> no longer persisted to localStorage. <code className="text-[10px] bg-[#0E100F] px-1">saveTradeLog</code> stubbed.</li>
                  <li><strong>W-L score counter:</strong> still saves as before — different localStorage key (<code className="text-[10px]">taraCallScorecards_v1</code>), separate state.</li>
                  <li><strong>Tara&rsquo;s own call log:</strong> unchanged — separate localStorage + Firestore-synced.</li>
                  <li><strong>Cleanup:</strong> any leftover trade entries from V6.2.3/V6.2.4 windows get cleared automatically on next load.</li>
                </ul>
              </section>

              {/* V6.2.4 — Mobile empty space fix */}
              <section className="mb-2 pb-3" style={{borderBottom:'1px solid '+T2_GOLD_GLOW}}>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-[9px] uppercase tracking-[0.18em] font-bold" style={{color:T2_GOLD}}>Mobile Empty-Space Fix</span>
                  <span className="text-[9px] uppercase tracking-wider text-[#E8E9E4]/30">2026.05.03</span>
                </div>
                <h3 className="font-serif text-2xl mb-2 tracking-tight text-white">Tara <span style={{color:T2_GOLD}}>6.2.4</span> — No More Dead Space</h3>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed">User screenshot showed ~400px of empty space between Quality Gate and the ENTERED UP/DOWN buttons on the mobile signal tab. Cause: <code className="text-[10px] bg-[#0E100F] px-1">mt-auto</code> on the &ldquo;-30% Stop Guard Sync&rdquo; block was pushing it to the bottom of a viewport-tall flex column. The intent (from a much earlier version) was filling desktop empty space; mobile got dead patches instead. Replaced with <code className="text-[10px] bg-[#0E100F] px-1">mt-4</code>. Buttons now sit naturally below Quality Gate with normal spacing on every screen size.</p>
              </section>
              ...
              <section className="mb-2 pb-3" style={{borderBottom:'1px solid '+T2_GOLD_GLOW}}>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-[9px] uppercase tracking-[0.18em] font-bold" style={{color:T2_GOLD}}>Multi-Timeframe Alignment</span>
                  <span className="text-[9px] uppercase tracking-wider text-[#E8E9E4]/30">2026.05.03</span>
                </div>
                <h3 className="font-serif text-2xl mb-2 tracking-tight text-white">Tara <span style={{color:T2_GOLD}}>6.2.1</span> — Same Logic You Use on TradingView</h3>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-3">User clarification: &ldquo;My TradingView chart isn&rsquo;t configured much. I just have those indicators enabled and switch between 1min/5min/15min to see how they align and take the call.&rdquo; Critical info — V6.2.0 only computed on 5m. That&rsquo;s a different signal than what the user actually trades. Fixed.</p>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-3 mb-2">What changed</div>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>Both indicators now compute on 1m, 5m, AND 15m candles.</strong> Each gives its own direction reading per timeframe.</li>
                  <li><strong>6 total signals:</strong> 3 timeframes × 2 indicators (Trend Channel + Grand Trend).</li>
                  <li><strong>Structural-led requires ≥4 of 6 to agree</strong> with the lock direction. Stronger version (5+ of 6 + meaningful 5m strength) is the &ldquo;perfect alignment&rdquo; you&rsquo;d see when all timeframes scream the same direction on TradingView.</li>
                  <li><strong>Single-timeframe agreement is no longer enough</strong> — the V6.2.0 logic was too lenient compared to your actual workflow.</li>
                </ul>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">UI: see alignment at a glance</div>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-2">Score Breakdown panel now shows three rows:</p>
                <ul className="list-disc pl-4 space-y-1 text-[11px]">
                  <li><strong>Structural</strong> — combined alignment (e.g. &ldquo;▼ DOWN · 5/6 align&rdquo; with &ldquo;gate&rdquo; status meaning structural-led can fire)</li>
                  <li><strong>Grand Tr.</strong> — per-timeframe arrows: <code className="text-[10px]">1m▼ 5m▼ 15m▼</code></li>
                  <li><strong>Trend Ch.</strong> — same per-timeframe layout, plus 5m channel position %</li>
                </ul>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mt-2">No more switching tabs to check alignment — it&rsquo;s right there on screen. Status hints: <strong>gate</strong> (4+/6 — structural-led can lock), <strong>edge</strong> (3/6 — informational), <strong>mixed</strong> (≤2/6 — no signal).</p>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">Engine Log addition</div>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed">New <code className="text-[10px] bg-[#0E100F] px-1">[STRUCT]</code> entries show the alignment count (e.g. &ldquo;5/6 align DOWN (1↑ vs 5↓)&rdquo;). Plus per-TF detail in the existing GRAND-TREND and CHANNEL entries.</p>

                <p className="text-xs text-[#E8E9E4]/55 leading-relaxed mt-4 italic">Net: structural-led now reflects what you actually trade off TradingView. Single-TF noise no longer triggers it. Cross-TF alignment is the threshold, same as your manual workflow.</p>
              </section>
              ...
              <section className="mb-2 pb-3" style={{borderBottom:'1px solid '+T2_GOLD_GLOW}}>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-[9px] uppercase tracking-[0.18em] font-bold" style={{color:T2_GOLD}}>Loosened Cap · Cleaner Engine Card</span>
                  <span className="text-[9px] uppercase tracking-wider text-[#E8E9E4]/30">2026.05.03</span>
                </div>
                <h3 className="font-serif text-2xl mb-2 tracking-tight text-white">Tara <span style={{color:T2_GOLD}}>6.1.1</span> — Cap, but Smarter</h3>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-3">Real-world test of V6.1.0 caught it: engine locked a slam-dunk DOWN at 2:16 (TRENDING DOWN, TRAJ -12, EXTREME, gap -18bps, posterior 92%) but Tara sat out at 90s. The 90s cap was too aggressive. Loosened plus added an override for genuinely super-clean conditions.</p>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-3 mb-2">New cap timing</div>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>15m windows:</strong> 90s → <strong style={{color:T2_GOLD}}>120s</strong> (2 min)</li>
                  <li><strong>5m windows:</strong> 45s → <strong style={{color:T2_GOLD}}>60s</strong> (1 min)</li>
                  <li>Still well within the user&rsquo;s stated &ldquo;2&ndash;3 min max,&rdquo; gives 30&ndash;45 more seconds for legitimate setups.</li>
                </ul>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">Super-confluence override past the cap</div>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>Past the 120s/60s cap, I can still lock — but only if conditions are genuinely super-clean.</strong> Two override paths:</li>
                  <li><strong>Path 1 · Super-confluent:</strong> tape ≥70% across 2+ windows + FGT-sign agrees + score-sign agrees + strike favorable + no Kalshi disagreement. All five conditions.</li>
                  <li><strong>Path 2 · Tape-led + favorable strike:</strong> tape ≥70% across 2+ windows AND price already on the right side by ≥10bps. The slam-dunk case.</li>
                  <li><strong>What this catches:</strong> the screenshot scenario. Tape was 79.1% / 76.7% sell, gap was -18bps DOWN, engine locked at 2:16 — exactly Path 2. Past V6.1.1, Tara would have locked here too.</li>
                  <li><strong>What it doesn&rsquo;t do:</strong> rescue marginal late commits. Single-signal, mixed-FGT, late-window setups still sit out — that&rsquo;s the V6.0.9 late-commit guard, still active.</li>
                </ul>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">Engine card text cleanup</div>
                <ul className="list-disc pl-4 space-y-1.5 text-[11px]">
                  <li><strong>&ldquo;Tara locked DOWN 33s ago at 8% conf. Gap: -18.1 bps. [12m 10s left]&rdquo;</strong> → <strong>&ldquo;Engine locked DOWN 33s ago · 8% conf · gap -18.1bps · 12m 10s left&rdquo;</strong></li>
                  <li><strong>Why:</strong> the engine prediction card was saying &ldquo;Tara locked&rdquo; even when Tara was sitting out. That card reflects engine state, not Tara&rsquo;s. Now correctly attributed.</li>
                  <li><strong>&ldquo;Tara loosens her standards each minute&rdquo;</strong> → <strong>&ldquo;Scanning for direction&rdquo;</strong> (was Tara behavior, doesn&rsquo;t apply to engine)</li>
                  <li>Format simplified throughout: dot separators (·) instead of brackets/periods, shorter prose.</li>
                </ul>

                <p className="text-xs text-[#E8E9E4]/55 leading-relaxed mt-4 italic">Net: cap stays predictable (120s ceiling on 15m), but obvious slam-dunk windows aren&rsquo;t missed. Engine card no longer claims to speak for Tara.</p>
              </section>
              ...
              <section className="mb-2 pb-3" style={{borderBottom:'1px solid '+T2_GOLD_GLOW}}>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-[9px] uppercase tracking-[0.18em] font-bold" style={{color:T2_GOLD}}>Tape-Led · Fast-Lock When Order Flow Is Unanimous</span>
                  <span className="text-[9px] uppercase tracking-wider text-[#E8E9E4]/30">2026.05.03</span>
                </div>
                <h3 className="font-serif text-2xl mb-2 tracking-tight text-white">Tara <span style={{color:T2_GOLD}}>6.0.7</span> — Tape Is the Signal</h3>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-3">User feedback: &ldquo;Why are we waiting 7&ndash;8 minutes on a decision? By then even a kid can tell. What&rsquo;s the use of an advanced bot?&rdquo; Right diagnosis. Their screenshot showed tape at 90% one-sided across ALL windows but Tara was sitting at WATCHING because q=11 fell below the 30 floor. That&rsquo;s the bug — tape was the signal and the q-floor was blocking it.</p>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-3 mb-2">The fix</div>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>New TAPE-LED tier.</strong> Fires when tape super-strong agrees with candidate direction (≥70% in 2 of 3 of the 15s/30s/60s windows) AND conviction ≥4pt AND no opposing flags AND strike actionable (≤30 bps adverse). Locks in 5 samples (~5s) — same speed as super-confluence.</li>
                  <li><strong>Bypasses q-floor and conv-floor.</strong> When tape is unanimous, q-floor (30) and conv-floor (10) are irrelevant. Quality score is an aggregate derived from many factors, often lagging behind actual order flow. If 90% of trades are one direction, waiting for the aggregate to catch up means you missed the entry.</li>
                  <li><strong>Cyan ⚡ tape-led badge</strong> on the call card so you can identify these locks visually.</li>
                  <li><strong>Separate learning bucket.</strong> Tape-led calls track WR independently in the Learnings modal — you&rsquo;ll see whether they actually deliver as expected.</li>
                  <li><strong>Tape-led also uses early-entry chirp sound</strong> (lighter than confluence arpeggio). Stereo cue still applies.</li>
                </ul>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">Why other gates still apply</div>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>Tape opposes still vetoes.</strong> If two tape windows show ≥60% the OTHER way, the call is still blocked. Tape-led requires tape pointing the same way as the candidate.</li>
                  <li><strong>Kalshi extreme disagreement still vetoes.</strong> If market is ≥90% certain the opposite way, tape-led is blocked.</li>
                  <li><strong>Tape-super-strong is a high bar.</strong> 2 of 3 windows at ≥70% is rare in choppy markets. This tier only fires when order flow is genuinely unanimous, not just leaning.</li>
                </ul>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">New tier ladder</div>
                <ul className="list-disc pl-4 space-y-1 text-[11px]">
                  <li>Super-confluence: 5 samples (~5s) — all four signals aligned</li>
                  <li><span style={{color:'#7DD3FC'}}>Tape-led: 5 samples (~5s)</span> — V6.0.7 NEW · tape ≥70% across windows, bypass q/conv floors</li>
                  ...
                  <li>Default: 90 samples (~90s)</li>
                  <li>Patient: 130 samples (~130s)</li>
                </ul>

                <p className="text-xs text-[#E8E9E4]/55 leading-relaxed mt-4 italic">Honest expectation: tape-led WR is unlikely to be as high as full confluence (estimate 65&ndash;75% based on tape-only signal logic). But entry timing is dramatically better — Kalshi will still be near 50/50 when these fire, not 80&ndash;90%. The Learnings modal will show real WR after ~10 calls so you can see whether the trade-off works for you. If tape-led WR drops below 60%, the confluence-bucket learning will dampen its confidence automatically.</p>
              </section>

              {/* V6.0.6 — Cap ETA to deadline */}
              <section className="mb-2 pb-3" style={{borderBottom:'1px solid '+T2_GOLD_GLOW}}>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-[9px] uppercase tracking-[0.18em] font-bold" style={{color:T2_GOLD}}>Bug Fix · ETA Math</span>
                  <span className="text-[9px] uppercase tracking-wider text-[#E8E9E4]/30">2026.05.03</span>
                </div>
                <h3 className="font-serif text-2xl mb-2 tracking-tight text-white">Tara <span style={{color:T2_GOLD}}>6.0.6</span> — ETA Can&rsquo;t Exceed the Window</h3>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-3">User caught it: Decision Clock was showing &ldquo;~17m 25s to lock&rdquo; on a window with only 9 minutes remaining. The rate-based ETA was mathematically correct (samples accumulating ~1 per 30s while needing 112 more) but nonsensical to display.</p>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>Cap ETA to deadline.</strong> When projected lock ETA exceeds the time-to-deadline (with a small buffer), the clock now shows the deadline countdown instead of the inflated lock ETA.</li>
                  <li><strong>New &ldquo;won&rsquo;t reach lock&rdquo; warning state.</strong> When ETA exceeds deadline, the Decision Clock turns amber and shows: &ldquo;7m 07s deadline · won&rsquo;t reach lock at current pace.&rdquo; The phase strip mirrors the same.</li>
                  <li><strong>Same cap for the phase strip countdown.</strong> Was &ldquo;decision in ~17m 25s&rdquo; — now &ldquo;7m 07s to decide&rdquo; with hint &ldquo;won&rsquo;t reach lock at current pace.&rdquo;</li>
                  <li><strong>What this tells you:</strong> Tara&rsquo;s seeing some directional pressure but signal quality is too weak to confirm in time. You can still act on the lean (WATCHING is showing) — just know that Tara won&rsquo;t formally commit this window unless conditions clear up dramatically.</li>
                </ul>
              </section>

              {/* V6.0.5 — WATCHING state + Decision Clock */}
              <section className="mb-2 pb-3" style={{borderBottom:'1px solid '+T2_GOLD_GLOW}}>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-[9px] uppercase tracking-[0.18em] font-bold" style={{color:T2_GOLD}}>5-State Model · Always-Visible Timer</span>
                  <span className="text-[9px] uppercase tracking-wider text-[#E8E9E4]/30">2026.05.03</span>
                </div>
                <h3 className="font-serif text-2xl mb-2 tracking-tight text-white">Tara <span style={{color:T2_GOLD}}>6.0.5</span> — Watching, Then Locking</h3>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-3">User feedback on V6.0.4: &ldquo;Still no timer of decision in seconds. If she has good confidence at the start, show me WATCHING DOWN or UP. When confidence builds, lock. If no confidence, just SCANNING.&rdquo; That&rsquo;s a better design than the 4-state model. V6.0.5 implements it.</p>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-3 mb-2">5 visible states (was 4)</div>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>SCANNING</strong> — gold, no direction. Conviction &lt; 5pt off neutral, or direction unclear. &ldquo;Looking for edge.&rdquo;</li>
                  <li><strong>WATCHING ▲ UP / ▼ DOWN</strong> — muted green / muted rose. Conviction ≥ 5pt with consistent direction. Tara sees the lean but hasn&rsquo;t committed. <em>NEW in V6.0.5.</em> Conviction shown inline next to the headline (e.g. &ldquo;▼ WATCHING DOWN · 10pt&rdquo;).</li>
                  <li><strong>LOCKED ▲ UP / ▼ DOWN</strong> — saturated green / saturated rose. Tara has committed; samples passed needSamples or endgame freeze fired. Confidence % shown inline.</li>
                  <li><strong>SITTING OUT</strong> — amber. Window resolved without commit (gate blocked or endgame freeze with insufficient signal).</li>
                </ul>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mt-3 mb-3">The transition is now smooth: <span className="font-bold">SCANNING → WATCHING (when lean appears) → LOCKED (when committed)</span> or <span className="font-bold">SITTING OUT (if window ends without commit)</span>. WATCHING uses muted versions of the LOCKED hues so &ldquo;leaning&rdquo; is visually distinct from &ldquo;committed.&rdquo;</p>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">Decision Clock</div>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>Dedicated bordered box</strong> with a ⏱ icon, visible during scanning and watching. Always shows a real time number.</li>
                  <li><strong>Forming:</strong> &ldquo;~25s to lock · 7m 30s deadline&rdquo; — real ETA from sample-rate measurement, with deadline as backup.</li>
                  <li><strong>Gate blocking / pre-form:</strong> &ldquo;7m 30s to decide · ~90s to lock once forming&rdquo; — deadline countdown + tier projection.</li>
                  <li><strong>Red urgency state:</strong> within 10 seconds of the deadline, the box turns rose-red.</li>
                  <li><strong>Deadline anchor:</strong> derived from the endgame freeze trigger (90s before close on 15m, 45s on 5m). After this point a snapshot is forced.</li>
                </ul>

                <p className="text-xs text-[#E8E9E4]/55 leading-relaxed mt-4 italic">Practical effect: when Tara has any meaningful directional read, you see WATCHING immediately — not the generic SCANNING. You can act on the lean for early Kalshi entry, or wait for the formal LOCK. The Decision Clock removes guesswork about timing. Tara&rsquo;s Record (W/L) still tracks only formally-committed calls.</p>
              </section>

              {/* V6.0.4 — Visible lean + dynamic countdown */}
              <section className="mb-2 pb-3" style={{borderBottom:'1px solid '+T2_GOLD_GLOW}}>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-[9px] uppercase tracking-[0.18em] font-bold" style={{color:T2_GOLD}}>Visible Lean · Live Countdown</span>
                  <span className="text-[9px] uppercase tracking-wider text-[#E8E9E4]/30">2026.05.03</span>
                </div>
                <h3 className="font-serif text-2xl mb-2 tracking-tight text-white">Tara <span style={{color:T2_GOLD}}>6.0.4</span> — Show the Lean, Show the Clock</h3>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-3">User feedback: &ldquo;Where&rsquo;s the countdown? She locks at halfway through the window — by then Kalshi has already priced the move in.&rdquo; Right diagnosis: Tara had all the developing info internally but the UI was hiding it behind a generic SCANNING placeholder.</p>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>Soft lean indicator on the call card.</strong> Once Tara sees ≥4pt of directional pressure, the card shows a small ▲ / ▼ line under the SCANNING headline with current conviction and what&rsquo;s blocking. Example: &ldquo;▼ DOWN lean · 7pt &mdash; not yet committed (need 3 more for forming).&rdquo; This breaks the strict no-direction-reveal rule for SCANNING because the user explicitly needs the lean info to enter Kalshi while odds are still actionable.</li>
                  <li><strong>Live gate reason as the call subtitle.</strong> Was the static &ldquo;Scanning for direction. She&rsquo;ll signal only on commit.&rdquo; Now shows the specific reason from the active gate — e.g., &ldquo;Coin-flip — posterior 53% only 3pt off neutral (need ±10)&rdquo; or &ldquo;Watching DOWN — FGT 0.4/4 below 1.0 threshold.&rdquo; You see exactly what would need to change for her to commit.</li>
                  <li><strong>Countdown shows progress earlier.</strong> Was waiting until samples ≥ 5 to show ETA — now shows from sample 2. When samples haven&rsquo;t started, shows the gate-block reason instead. No more dead air during the early window.</li>
                  <li><strong>Phase hint surfaces dynamic info.</strong> The small text next to the phase bar now mirrors the gate reason (truncated) instead of the generic &ldquo;no clear direction yet.&rdquo;</li>
                </ul>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">Why she still locks late in choppy markets</div>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-2">Honest answer: when the market is genuinely choppy, conviction stays under the floor for several minutes. Tara correctly waits for clarity. The lock-at-halfway pattern is real and correct in those windows — there wasn&rsquo;t a high-confidence read available earlier. With this update you&rsquo;ll now see her developing lean as it forms, even if she doesn&rsquo;t formally commit until later. That&rsquo;s the actionable info you needed.</p>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed">Workflow: when the lean shows ≥6pt sustained for 15&ndash;30 seconds and Kalshi is still near 50/50, enter early on the lean. Tara&rsquo;s formal lock will follow when conditions confirm. If the lean flips, you exit — that&rsquo;s the trade-off you accepted for better entry pricing. Tara&rsquo;s Record (W/L) below still reflects only her formally-committed calls, so you can audit her actual discipline separately from your early-lean entries.</p>
              </section>
              ...
              <section className="mb-2 pb-3" style={{borderBottom:'1px solid '+T2_GOLD_GLOW}}>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-[9px] uppercase tracking-[0.18em] font-bold" style={{color:T2_GOLD}}>Major Release · Tape Truth + Confluence</span>
                  <span className="text-[9px] uppercase tracking-wider text-[#E8E9E4]/30">2026.05.03</span>
                </div>
                <h3 className="font-serif text-2xl mb-2 tracking-tight text-white">Tara <span style={{color:T2_GOLD}}>6.0</span> — Tape is the Truth, Confluence is the Trigger</h3>
                <p className="text-xs text-[#E8E9E4]/70 leading-relaxed mb-3">A philosophical shift, not a feature pack. V5.6 brought multi-device sync and self-learning. V5.7 polished correctness and surfaces. V6.0 is when the decision logic itself was rebuilt around a single principle: the tape is the only signal that can&rsquo;t lie. Everything else is derived from past prices.</p>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">Decision logic</div>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>Tape-opposes veto.</strong> When two of the three tape windows (15s/30s/60s) show ≥60% in the OPPOSITE direction of the candidate call, the lock is vetoed. ≥70% the other way is a hard veto regardless of Kalshi. The case that motivated it: tape was 79% buy and Tara wanted to lock DOWN — that&rsquo;s fighting actual order flow. No more.</li>
                  <li><strong>Confluence detection + fast-lock tier.</strong> When tape + FGT-sign + score-sign + strike-position all agree, it&rsquo;s &ldquo;confluence.&rdquo; All four PLUS tape super-strong PLUS no Kalshi disagreement is &ldquo;super-confluence.&rdquo; Confluent setups lock in 8 samples (~8s); super-confluent in 5. Single-signal setups still go through the patient 100–150 sample default.</li>
                  <li><strong>FGT horizon cap.</strong> Each timeframe&rsquo;s forecast was projecting hours ahead while the trading window was minutes. Now each timeframe&rsquo;s forecast bars clamp to remaining-window seconds. A 5m FGT with 3 minutes left projects 0 bars forward — neutral. Stops the structural misread that flipped marginal-up reads into confident-down locks at boundaries.</li>
                  <li><strong>Kalshi extreme-disagreement gate.</strong> When market is ≥90% certain the other way and Tara wants to call opposite, the call requires BOTH tape strong-agree AND FGT-sign-agree. Otherwise SIT_OUT. The market is right often enough that fighting 90%+ certainty without overwhelming evidence is a losing bet.</li>
                  <li><strong>3m FGT removed, weights redistributed.</strong> 3m was dead weight per its own comment — often invalid, only 0.5 weight, redundant given 1m and 5m bracket it. Now 1m=1.0, 5m=1.7, 15m=1.3 (still totals 4.0 so alignment thresholds mean the same thing).</li>
                  <li><strong>Engine vocabulary distinct from Tara&rsquo;s.</strong> Engine general prediction reads UP&nbsp;-&nbsp;CONFIRMED / DOWN&nbsp;-&nbsp;CONFIRMED / SIGNAL CLEARED. The word LOCKED is reserved for Tara&rsquo;s own commit.</li>
                </ul>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">Self-learning</div>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>Confluence-bucket learning.</strong> Every resolved call buckets into super-confluence / confluence / single-signal. After 5+ calls in a bucket, Tara adjusts confidence per-bucket based on observed WR (≥80% → +8 conf; ≥65% → +4; &lt;55% → −3; &lt;45% → −6). Stacks with the existing regime+direction adjustment. Visible in the Learnings modal.</li>
                  <li><strong>15s cooldown after release on confluence.</strong> The post-release cooldown was 45s flat. Now it drops to 15s when the current candidate is confluent. Re-engaging faster when all signals re-align is justified — confluence carries enough orthogonal evidence.</li>
                  <li><strong>Per regime+direction speed and confidence learning.</strong> (V5.6.8 carried forward.) Combos with ≥75% WR over 5+ calls get 20% faster locks and +5 confidence. Combos with &lt;40% get 20% slower locks and −5 confidence. Tara picks better, doesn&rsquo;t call less.</li>
                </ul>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">Settlement and memory</div>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>Settlement waits 8s for Kalshi&rsquo;s official close.</strong> At rollover, scoring defers 8 seconds while Kalshi publishes the new window&rsquo;s strike — that value equals the just-closed window&rsquo;s true close per Kalshi&rsquo;s feed. Falls back to the rollover-instant spot snapshot if Kalshi didn&rsquo;t refresh in time. Each resolved row is tagged with whether kalshi or spot settled it.</li>
                  <li><strong>Memory and Stats rows show window period, strike, closing price.</strong> Each call now displays the window time range (e.g. 09:00&ndash;09:15), strike, and close price on a second line.</li>
                  <li><strong>Scorecards derived from log.</strong> W/L/SITOUT counters compute directly from the call log instead of being separately incremented. Multi-device counting can no longer drift — opening Tara on PC and tab simultaneously can&rsquo;t double-count results.</li>
                </ul>

                <div className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#E8E9E4]/55 mt-4 mb-2">Sync and reliability</div>
                <ul className="list-disc pl-4 space-y-1.5">
                  <li><strong>Firestore sync, invisible.</strong> Call log, lock state, learnings, past windows all sync across devices in real time. No badges, no buttons — same Tara on every screen.</li>
                  <li><strong>Per-window dedup.</strong> Each call has a windowId derived from the bucket-start timestamp. Two devices committing the same window produce one log entry, not two.</li>
                  <li><strong>Honest decision ETA.</strong> The countdown is computed from actual sample-accumulation rate, not a fictional 1Hz assumption. When samples aren&rsquo;t accruing, the display says &ldquo;waiting for stronger signal&rdquo; instead of pretending a lock is imminent.</li>
                  <li><strong>All time classifications local.</strong> Hourly stats, sessions, day-of-week buckets all use your local clock. Macro events stay UTC since those are globally fixed.</li>
                </ul>

                <p className="text-xs text-[#E8E9E4]/55 leading-relaxed mt-4 italic">Practical effect: more calls, better picks. Fewer single-signal locks (vetoed when tape opposes). Faster locks on confluence. Same patient default for ambiguous setups. The confluence learning makes Tara calibrate her own confidence over time so the displayed % reflects what&rsquo;s actually been working.</p>
              </section>
