...
const LiveChart = ({ resolution, currentPrice, targetMargin, showCandles, rugPullActive, showOverlays, fallbackData, projections, prediction, liquidations }) => {
  ...
  useEffect(() => {
      const fetchKlines = async () => {
          try {
              ...
              } catch(e2) { console.error("All Chart APIs Blocked:", e2); }
          }
      };
      fetchKlines();
  ...
  useEffect(() => {
    ...
    try {
        ...
        chart.subscribeCrosshairMove((param) => {
            if (!param.time || param.point.x < 0 || param.point.y < 0) { setCrosshairData(null); return; }
            const cData = param.seriesData.get(mainSeries);
            const vData = param.seriesData.get(volSeries);
            if (cData) {
                setCrosshairData({ x: param.point.x, y: param.point.y,
                    ...
                    vol: vData ? vData.value : 0, time: param.time });
            }
        });
  ...
  useEffect(() => {
    ...
    try {
        ...
        const cleanData = activeData.map(d => ({ ...d, t: getUnix(d.time) }))
        ...
        const mainData = [], volData = [];
        for (const d of uniqueData) {
            if (showCandles) mainData.push({ time: d.t, open: d.o, high: d.h, low: d.l, close: d.c });
            else mainData.push({ time: d.t, value: d.c });
        ...
        if (showOverlays) {
            ...
            const e9Arr = calcEMA(closes, 9), e21Arr = calcEMA(closes, 21);
            const e9Data=[], e21Data=[], bbuData=[], bblData=[];
            ...
            chartRefs.current.e9.setData(e9Data); chartRefs.current.e21.setData(e21Data);
            chartRefs.current.bbu.setData(bbuData); chartRefs.current.bbl.setData(bblData);
        } else {
  ...
  useEffect(() => {
      ...
      try {
          const markers = [];
          liquidations.forEach(l => { markers.push({ time: Math.floor(l.time / 1000), position: l.side === 'BUY' ? 'belowBar' : 'aboveBar', color: l.side === 'BUY' ? '#34d399' : '#fb7185', shape: l.side === 'BUY' ? 'arrowUp' : 'arrowDown', text: 'LIQ' }); });
          const uniqueMarkers = []; const seen = new Set();
          markers.sort((a,b)=>a.time - b.time).forEach(m => { if(!seen.has(m.time)) { seen.add(m.time); uniqueMarkers.push(m); } });
  ...
  useEffect(() => {
      ...
      try {
          ...
          const allProjPoints = [];
          projections.forEach(p => { if(p.timeline) p.timeline.forEach(t => allProjPoints.push({time: t.timestamp, value: t.price})); });
          const predData = [{ time: lastHistoricalTime, value: lastPrice }, ...allProjPoints];
          const uniquePredData = []; const seen = new Set();
          for (const d of predData) { if (!seen.has(d.time) && !isNaN(d.time) && d.time >= lastHistoricalTime) { seen.add(d.time); uniquePredData.push(d); } }
  ...
  useEffect(() => {
    ...
    if (!LineStyle) return;
    if (chartRefs.current.strike) { chartRefs.current.mainSeries.removePriceLine(chartRefs.current.strike); chartRefs.current.strike = null; }
    if (targetMargin > 0) {
        try {
            chartRefs.current.strike = chartRefs.current.mainSeries.createPriceLine({ price: targetMargin, color: '#818cf8', lineWidth: 2, lineStyle: LineStyle.Dashed, axisLabelVisible: true, title: 'STRIKE' });
  ...
  useEffect(() => {
    ...
    try {
        ...
        chartRefs.current.chart.applyOptions({ layout: { background: { type: ColorType.Solid, color: rugPullActive ? 'rgba(251, 113, 133, 0.05)' : 'transparent' } } });
  ...
  return (
    <div className="w-full h-full relative" style={{ touchAction: 'none' }}>
        ...
        {crosshairData && (
            <div className="absolute z-20 bg-[#181A19]/95 border border-[#E8E9E4]/15 rounded shadow-xl p-2 pointer-events-none transform -translate-x-1/2 -translate-y-[120%]" style={{ left: crosshairData.x, top: crosshairData.y }}>
                <div className="text-[#E8E9E4] text-[10px] font-mono leading-tight whitespace-nowrap">
                    ...
                    <div className="mt-1 pt-1 border-t border-[#E8E9E4]/10 text-purple-300"><span className="opacity-50">V:</span> {crosshairData.vol.toFixed(2)}</div>
                </div>
            </div>
        )}
...
function TaraApp() {
  ...
  const [regimeMemory, setRegimeMemory] = useState({
      "TRENDING UP": { wins: 0, losses: 0 }, "TRENDING DOWN": { wins: 0, losses: 0 },
      "HIGH VOL CHOP": { wins: 0, losses: 0 }, "SHORT SQUEEZE": { wins: 0, losses: 0 },
      "LONG SQUEEZE": { wins: 0, losses: 0 }, "RANGE/CHOP": { wins: 0, losses: 0 }
  });
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedScore) { const parsed = JSON.parse(savedScore); if (parsed && typeof parsed['15m'] === 'object' && typeof parsed['15m'].wins === 'number') setScorecards(parsed); }
      else setScorecards({ '15m': { wins: 146, losses: 104 }, '5m': { wins: 10, losses: 7 } });
    ...
    } catch (e) { setScorecards({ '15m': { wins: 146, losses: 104 }, '5m': { wins: 10, losses: 7 } }); }
  }, []);
  ...
  useEffect(() => {
      if (userPosition === null) { peakOfferRef.current = 0; activeAdviceRef.current = "HOLD FIRM"; }
      else { const offer = parseFloat(currentOffer) || 0; if (offer > peakOfferRef.current) peakOfferRef.current = offer; }
  ...
  const getHistoricPrice = (msAgo) => {
      ...
      let closest = mem[0];
      for (let i = mem.length - 1; i >= 0; i--) { if (mem[i].time <= target) { closest = mem[i]; break; } }
      return closest.p;
  };
  ...
  const broadcastToDiscord = async (type, data) => {
    ...
    try {
      let embed = {};
      if (type === 'SIGNAL') {
          embed = { title: `${data.dir === 'UP' ? '🟢' : '🔴'} TARA SIGNAL: ${data.dir}`, color: data.dir === 'UP' ? 3404125 : 16478549,
              fields: [{ name: "BTC Price", value: `$${data.price.toFixed(2)}`, inline: true }, { name: "Strike", value: `$${data.strike.toFixed(2)}`, inline: true }, { name: "Gap", value: `${data.gap.toFixed(2)} bps`, inline: true }, { name: "Round Clock", value: data.clock, inline: true }],
              timestamp: new Date().toISOString() };
      } else if (type === 'LOCK') {
          embed = { title: `TARA — ${data.dir} LOCKED`, color: data.dir === 'UP' ? 3404125 : 16478549,
          ...
          embed = { title: `TARA ROUND CLOSED: ${data.window}`, color: data.won ? 3404125 : 16478549,
              description: `**Result:** ${data.won ? 'WIN' : 'LOSS'}\n**Prediction:** ${data.prediction}\n**Closing Price:** $${data.price.toFixed(2)}\n**Regime Recorded:** ${data.regime}`,
              timestamp: new Date().toISOString() };
      }
      await fetch(discordWebhook, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: "Tara Terminal", avatar_url: "https://i.imgur.com/8nLFCVP.png", embeds: [embed] }) });
    } catch (e) {}
  };
  ...
  const handleWindowToggle = (type) => {
    ...
    setWindowType(String(type)); lockedPredictionRef.current = "SEARCHING..."; activeCallRef.current = { prediction: "SEARCHING...", strike: currentPrice };
    hasReversedRef.current = false; taraAdviceRef.current = "SEARCHING..."; activeAdviceRef.current = "HOLD FIRM";
    setUserPosition(null); setPositionEntry(null); setManualAction(null); setCurrentOffer(""); setBetAmount(0); setMaxPayout(0); lastWindowRef.current = ""; setForceRender(prev => prev + 1);
  };

  const updateScore = (type, winOrLoss, amount) => {
    setScorecards(prev => ({ ...prev, [type]: { ...prev[type], [winOrLoss]: Math.max(0, (Number(prev[type]?.[winOrLoss]) || 0) + amount) } }));
  };
    ...
    if (timeState.nextWindow && timeState.nextWindow !== lastWindowRef.current) {
      if (currentPrice !== null) {
        if (lastWindowRef.current !== "") {
          const pc = taraAdviceRef.current; let won = false; let tradeActive = false;
          if (pc.includes("UP - LOCKED")) { tradeActive = true; if (currentPrice > targetMargin) { updateScore(windowType, 'wins', 1); won = true; } else if (currentPrice < targetMargin) updateScore(windowType, 'losses', 1); }
          else if (pc.includes("DOWN - LOCKED")) { tradeActive = true; if (currentPrice < targetMargin) { updateScore(windowType, 'wins', 1); won = true; } else if (currentPrice > targetMargin) updateScore(windowType, 'losses', 1); }
          if (tradeActive) {
             setRegimeMemory(prev => { const updated = { ...prev }; const r = lastRegimeRef.current || "RANGE/CHOP"; if (!updated[r]) updated[r] = { wins: 0, losses: 0 }; if (won) updated[r].wins += 1; else updated[r].losses += 1; return updated; });
          }
          if (pc.includes("LOCKED")) broadcastToDiscord('CLOSE', { window: windowType, won, prediction: pc, regime: lastRegimeRef.current, price: currentPrice });
        }
        setTargetMargin(currentPrice); lockedPredictionRef.current = "SEARCHING..."; activeCallRef.current = { prediction: "SEARCHING...", strike: currentPrice };
        hasReversedRef.current = false; taraAdviceRef.current = "SEARCHING..."; activeAdviceRef.current = "HOLD FIRM";
        setUserPosition(null); setPositionEntry(null); lastWindowRef.current = timeState.nextWindow; setManualAction(null);
        tickHistoryRef.current = []; setCurrentOffer(""); setBetAmount(0); setMaxPayout(0); hasSetInitialMargin.current = true;
      }
    }
  ...
  useEffect(() => {
    ...
    const fetchSpotPrice = async () => {
      try {
        ...
        if (dataTicker.price) {
          const p = parseFloat(dataTicker.price); const now = Date.now();
    ...
    const fetchHeavyData = async () => {
      try {
        ...
        try {
          ...
          if (resCB.ok) { const dataCB = await resCB.json(); if (Array.isArray(dataCB)) setHistory(dataCB.slice(0, 60).map(c => ({ time: c[0], l: parseFloat(c[1]), h: parseFloat(c[2]), o: parseFloat(c[3]), c: parseFloat(c[4]), v: parseFloat(c[5]) }))); }
        } catch (e) {}
        try {
  ...
  useEffect(() => { if (!hasSetInitialMargin.current && currentPrice) { setTargetMargin(currentPrice); hasSetInitialMargin.current = true; } }, [currentPrice]);
  ...
  useEffect(() => {
    ...
    if (showWhaleAlerts && whaleLog.length > 0) { const lastWhale = whaleLog[0]; syntheticNews.push({ title: `🐋 WHALE: Massive Market ${lastWhale.side} on ${lastWhale.src} Futures.`, type: 'whale' }); }
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      setTimeState({ currentTime: String(currentTime), startWindow: String(startWindowStr), nextWindow: String(nextWindowStr), minsRemaining: Math.floor(diffMs / 60000), secsRemaining: Math.floor((diffMs % 60000) / 1000), currentHour: now.getHours() });
    };
    updateTime();
  ...
  const analysis = useMemo(() => {
    try {
        ...
        let baseProb = 50, reasoning = [], regime = "RANGE/CHOP";
        ...
        if (gapMag > 15) { gapEffect += Math.sign(realGapBps) * Math.pow(gapMag - 10, 1.4) * (is15m ? 0.4 : 0.6); reasoning.push(`[GRAVITY] Extreme gap (${realGapBps.toFixed(0)}bps) heavily skewing odds.`); }
        baseProb += gapEffect;
        if (drift1m_bps > 0 && vwapGapBps < -5) { baseProb -= 15; reasoning.push(`[VWAP] Price below VWAP. Suppressing UP bias.`); }
        if (drift1m_bps < 0 && vwapGapBps > 5) { baseProb += 15; reasoning.push(`[VWAP] Price above VWAP. Suppressing DOWN bias.`); }
        if (is15m) {
            baseProb += drift5m_bps * 0.6 + drift1m_bps * 0.4; baseProb += (aggrFlow * 20);
            ...
            baseProb += v30s_bps * 1.5 + drift1m_bps * 1.0 + drift5m_bps * 0.5; baseProb += (aggrFlow * 30);
        ...
        if (retailShorting && whalesBuying) { regime = "SHORT SQUEEZE"; upThreshold = 60; downThreshold = 20; baseProb += 15; }
        else if (retailLonging && whalesSelling) { regime = "LONG SQUEEZE"; upThreshold = 80; downThreshold = 40; baseProb -= 15; }
        else if (isCleanTrendUp) { regime = "TRENDING UP"; upThreshold = 64; downThreshold = 25; }
        else if (isCleanTrendDn) { regime = "TRENDING DOWN"; upThreshold = 75; downThreshold = 36; }
        else if (isHighVol) { regime = "HIGH VOL CHOP"; upThreshold = 75; downThreshold = 25; reasoning.push(`[REGIME] High Volatility. Enforcing strict entry thresholds.`); }
        const mem = regimeMemory ? regimeMemory[regime] : null;
        if (mem && (mem.wins + mem.losses) >= 3) {
            const wr = mem.wins / (mem.wins + mem.losses);
            if (wr < 0.45) { upThreshold += 6; downThreshold -= 6; reasoning.push(`[MEMORY] Low historical WR (${(wr*100).toFixed(0)}%) in ${regime}. Adapting thresholds.`); }
            else if (wr > 0.65) { upThreshold -= 4; downThreshold += 4; reasoning.push(`[MEMORY] High historical WR (${(wr*100).toFixed(0)}%) in ${regime}. Aggressive entry enabled.`); }
        }
        lastRegimeRef.current = regime;
        let posterior = Math.max(1, Math.min(99, baseProb));
        if (realGapBps < -35) { posterior = Math.min(posterior, 20); reasoning.push(`[CAP] Deep underwater. UP odds strictly capped at 20%.`); }
        else if (realGapBps < -15) { posterior = Math.min(posterior, 40); }
        else if (realGapBps > 35) { posterior = Math.max(posterior, 80); reasoning.push(`[CAP] High profit. UP odds strictly floored at 80%.`); }
        else if (realGapBps > 15) { posterior = Math.max(posterior, 60); }
        if (isPostDecay && !isEndgameLock) reasoning.push(`[POST-DECAY] Prediction confidence rising.`);
        if (reasoning.length < 4) reasoning.push(`[SYS] Price hovering near baseline. Awaiting structural signal.`);
        ...
        let tradeAction = "WAITING / SKIP", tradeReason = "Awaiting structural confirmation to signal.";
        ...
        if ((isUP || isDN) && betAmount > 0 && maxPayout > betAmount) {
            const b = (maxPayout - betAmount) / betAmount; const p = currentOdds / 100; const q = 1 - p;
            const k = ((p * b) - q) / b; kellyPct = Math.max(0, (k / 2) * 100);
        }
        if (userPosition === null) {
            if (isRugPull && showRugPullAlerts) { tradeAction = "RUG PULL DETECTED"; tradeReason = `Massive liquidity collapse. Abort longs immediately.`; actionColor = "text-rose-500"; actionBg = "bg-rose-500/20 border-rose-500/50 animate-pulse shadow-[0_0_20px_rgba(225,29,72,0.4)]"; }
            else if (isCalibrating) { tradeAction = "QUICK SCAN"; tradeReason = "New window started. Executing rapid scan of market structure..."; actionColor = "text-amber-400"; }
            else if (isEndgameLock && !activePrediction.includes("LOCKED")) { tradeAction = "WINDOW CLOSED"; tradeReason = "Late in round. Entering now is unsafe."; actionColor = "text-amber-400"; actionBg = "bg-amber-500/10 border-amber-500/30"; }
            else if (activePrediction.includes("LOCKED")) {
                tradeAction = `ENTRY SIGNAL: ${isUP ? 'UP' : 'DOWN'}`; tradeReason = `Quant composite supports entry. Execute now. ${metricsStr}`;
                ...
                tradeAction = "BIAS FORMING"; tradeReason = isEarlyWindow ? `Quick scan complete. Early bias leans ${isUP ? 'UP' : 'DOWN'}, searching for firm lock... ${metricsStr}` : `Bias detected but momentum is insufficient for a hard lock. ${metricsStr}`;
                actionColor = "text-amber-400"; actionBg = "bg-amber-500/10 border-amber-500/30";
            } else { tradeAction = "ANALYZING"; tradeReason = isEarlyWindow ? "Volume profile is neutral. Searching order book and tape for a lockable bias..." : "Odds below entry threshold. Waiting for cleaner setup to lock."; actionColor = "text-zinc-400"; actionBg = "bg-zinc-500/10 border-zinc-500/30"; }
        } else {
            const peak = peakOfferRef.current; const drawdownFromPeak = peak > 0 ? ((peak - offerVal) / peak) : 0;
            const momentumAgainst = (isUP && tickSlope < 0) || (isDN && tickSlope > 0);
            const isBleeding = currentOdds < 45, isHardReversal = currentOdds < 20;
            ...
            if (positionStatus?.isStopHit) currentAdvice = "30% STOP HIT";
            else if (isRugPull && showRugPullAlerts && isUP) currentAdvice = "RUG PULL DETECTED";
            else if (stopBreached && adverseAccel) currentAdvice = "CUT LOSSES NOW";
            else if (peak > (betAmount * 1.1) && drawdownFromPeak > 0.08) currentAdvice = "TRAILING STOP HIT";
            else if (offerVal > 0 && livePnL < -2 && momentumAgainst) currentAdvice = "CUT LOSSES NOW";
            else if (offerVal > betAmount && livePnL > betAmount * 0.15 && momentumAgainst) currentAdvice = "SCALP PROFIT";
            else if (offerVal > betAmount && momentumAgainst && currentOdds < 65) currentAdvice = "SECURE PROFIT";
            else if (isHardReversal && !hasReversedRef.current) currentAdvice = "REVERSE POSITION";
            else if (isHardReversal && hasReversedRef.current) currentAdvice = "CUT LOSSES NOW";
            else if (isBleeding) currentAdvice = "CUT LOSSES";
            else if (currentOdds >= 85 && offerVal === 0) currentAdvice = "MAX PROFIT REACHED";
            else currentAdvice = "HOLD FIRM";
            activeAdviceRef.current = String(currentAdvice); tradeAction = String(currentAdvice);
            if (tradeAction === "30% STOP HIT") { tradeReason = `Position breached -30% limit. Execute emergency exit. ${metricsStr}`; actionColor = "text-rose-500"; actionBg = "bg-rose-500/20 border-rose-500/50 animate-pulse shadow-[0_0_20px_rgba(225,29,72,0.4)]"; hasAction = true; actionButtonLabel = "EXIT (STOP LOSS)"; actionTarget = "SIT OUT"; }
            else if (tradeAction === "RUG PULL DETECTED") { tradeReason = `Massive liquidity collapse. Abort longs immediately. ${metricsStr}`; actionColor = "text-rose-500"; actionBg = "bg-rose-500/20 border-rose-500/50 animate-pulse shadow-[0_0_20px_rgba(225,29,72,0.4)]"; hasAction = true; actionButtonLabel = "EMERGENCY CASHOUT"; actionTarget = "SIT OUT"; }
            else if (tradeAction === "TRAILING STOP HIT") { tradeReason = `Price retreated 8% from local peak. Lock in remaining profit. ${metricsStr}`; actionColor = "text-emerald-300"; actionBg = "bg-emerald-500/10 border-emerald-500/30 animate-pulse"; hasAction = true; actionButtonLabel = "EXECUTE CASHOUT"; actionTarget = "CASH"; }
            else if (tradeAction === "SCALP PROFIT") { tradeReason = `Decent profits achieved (+15%). Momentum stalling. Scalp it. ${metricsStr}`; actionColor = "text-emerald-300"; actionBg = "bg-emerald-500/10 border-emerald-500/30 animate-pulse"; hasAction = true; actionButtonLabel = "SCALP CASHOUT"; actionTarget = "CASH"; }
            else if (tradeAction === "SECURE PROFIT") { tradeReason = `Profit peaked at $${peak}. Momentum is flipping hard against you. Exit. ${metricsStr}`; actionColor = "text-emerald-300"; actionBg = "bg-emerald-500/10 border-emerald-500/30 animate-pulse"; hasAction = true; actionButtonLabel = "EXECUTE CASHOUT"; actionTarget = "CASH"; }
            else if (tradeAction === "CUT LOSSES NOW" || tradeAction === "CUT LOSSES") { tradeReason = `Position bleeding past stop & acceleration is adverse. Exit immediately. ${metricsStr}`; actionColor = "text-rose-500"; actionBg = "bg-rose-500/10 border-rose-500/30"; hasAction = true; actionButtonLabel = "EXECUTE CASHOUT"; actionTarget = "SIT OUT"; }
            else if (tradeAction === "REVERSE POSITION") { tradeReason = `Trend collapsed. Switch position. 1 Switch Limit. ${metricsStr}`; actionColor = "text-amber-400"; actionBg = "bg-amber-500/10 border-amber-500/30 shadow-[0_0_15px_rgba(251,191,36,0.2)]"; hasAction = true; actionButtonLabel = `REVERSE TO '${isUP ? 'DOWN' : 'UP'}'`; actionTarget = isUP ? "DOWN" : "UP"; }
            else if (tradeAction === "MAX PROFIT REACHED") { tradeReason = `Odds > 85%. Stand by to lock in gains. ${metricsStr}`; actionColor = "text-emerald-300"; actionBg = "bg-emerald-500/10 border-emerald-500/30"; hasAction = true; actionButtonLabel = "EXECUTE CASHOUT (PROFIT)"; actionTarget = "CASH"; }
            else { tradeReason = `Holding position. Velocity supports trend. ${metricsStr}`; actionColor = "text-emerald-400"; actionBg = "bg-emerald-500/10 border-emerald-500/20"; }
        }
        ...
        const generateTimeline = (intervalMin, steps) => {
            const out = []; const intervalMs = intervalMin * 60 * 1000; const now = Date.now();
            let nextT = Math.ceil(now / intervalMs) * intervalMs;
            if (nextT - now < intervalMs * 0.1) nextT += intervalMs;
            const timeZoneOpt = useLocalTime ? undefined : { timeZone: 'America/New_York' };
            for (let i=0; i<steps; i++) {
                const stepT = nextT + (i * intervalMs); const diffMin = (stepT - now) / 60000;
                ...
                try { timeStr = dateObj.toLocaleTimeString('en-US', { ...timeZoneOpt, hour12: true, hour: 'numeric', minute: '2-digit' }); } catch(e) {}
                out.push({ timeStr, timestamp: Math.floor(stepT/1000), price: p });
            }
            return out;
        };
        const t5 = generateTimeline(5, 8), t15 = generateTimeline(15, 8), t60 = generateTimeline(60, 8);
        projections = [
        ...
        return {
          ...
          prediction: String(activePrediction), predictionReason: String(predictionReason),
          reasoning: reasoning.map(r => String(r)), textColor: String(textColor),
          rawProbAbove: Number(posterior), tradeAction: String(tradeAction), tradeReason: String(tradeReason),
          actionColor: String(actionColor), actionBg: String(actionBg), hasAction: Boolean(hasAction),
          actionButtonLabel: String(actionButtonLabel), actionTarget: String(actionTarget),
          realGapBps: Number(realGapBps), clockSeconds: Number(clockSeconds), isSystemLocked: Boolean(isEndgameLock),
          atrBps: Number(atrBps), livePnL: Number(livePnL), liveEstValue: Number(liveEstValue),
          projections: projections, regime: String(regime), aggrFlow: Number(aggrFlow), bb: bb,
          kellyPct: Number(kellyPct), isPostDecay: Boolean(isPostDecay), isRugPull: Boolean(isRugPull)
        };
    } catch (err) {
        return { prediction: "ERROR", tradeAction: "MATH CRASH", tradeReason: err.message || String(err), rawProbAbove: 50, projections: [], reasoning: [err.stack || String(err)], textColor: "text-rose-500", actionColor: "text-rose-500", actionBg: "bg-rose-500/10", regime: "ERROR" };
    }
  ...
  const executeManualAction = (actionLabel, targetState) => {
    ...
    if (targetState === "UP" || targetState === "DOWN") { handleManualSync(targetState); return; }
    if (targetState === "CASH" || targetState === "SIT OUT") { setUserPosition(null); setPositionEntry(null); }
    ...
    if (targetState) { lockedPredictionRef.current = targetState === "CASH" ? "SIT OUT" : String(targetState); setForceRender(prev => prev + 1); setCurrentOffer(""); }
  };

  const handleManualSync = (dir) => {
    if (userPosition !== null && userPosition !== dir) hasReversedRef.current = true;
    if (userPosition === dir) { lockedPredictionRef.current = "SEARCHING..."; activeCallRef.current = { prediction: "SEARCHING...", strike: targetMargin }; setUserPosition(null); setPositionEntry(null); setForceRender(prev => prev + 1); return; }
    lockedPredictionRef.current = String(dir); activeCallRef.current = { prediction: String(dir), strike: targetMargin }; setUserPosition(String(dir));
    if(currentPrice) {
      ...
      broadcastToDiscord('LOCK', { dir, price: currentPrice, strike: targetMargin, gap: gapBps, clock: `${timeState.minsRemaining}m ${timeState.secsRemaining}s` });
    }
  ...
  const handleChatSubmit = (e) => {
    if (e.key === 'Enter' && chatInput.trim()) {
      ...
      setTimeout(() => {
        let reply = ""; const ut = userText.toLowerCase();
        if (ut.includes("/broadcast")) {
            const gapBps = targetMargin > 0 ? ((currentPrice - targetMargin) / targetMargin) * 10000 : 0;
            const dir = analysis?.prediction.includes("UP") ? "UP" : analysis?.prediction.includes("DOWN") ? "DOWN" : "SIT OUT";
            broadcastToDiscord('SIGNAL', { dir, price: currentPrice, strike: targetMargin, gap: gapBps, clock: `${timeState.minsRemaining}m ${timeState.secsRemaining}s` });
            reply = `Signal broadcasted to Discord.`;
        } else if (ut.includes("why") || ut.includes("explain")) { reply = `Currently, my posterior for UP is ${Number(analysis?.rawProbAbove || 0).toFixed(1)}%. We are in a ${String(analysis?.regime || 'CHOP')} regime.`; }
        else if (ut.includes("whale")) { reply = whaleLog.length>0 ? whaleLog.slice(0,10).map(w=>{const d=new Date(w.time);return`${d.toLocaleTimeString('en-US',{hour12:false,hour:'2-digit',minute:'2-digit',second:'2-digit'})} ${w.src} ${w.side} $${(w.usd/1000).toFixed(0)}K @ $${w.price.toFixed(0)}`;}).join('\n') : "No whales yet."; }
        else if (ut.includes("position") || ut.includes("stop")) { reply = positionStatus ? `Position: ${positionStatus.side} @ $${positionStatus.entry.toFixed(2)}\nPnL: ${positionStatus.pnlPct>0?'+':''}${positionStatus.pnlPct.toFixed(2)}% \n${positionStatus.isStopHit?'🚨 STOP HIT — EXIT NOW':'Stop not hit.'}` : "No active position."; }
        else if (ut.includes("session")) { reply = `Active: ${marketSessions.sessions.map(s=>`${s.flag} ${s.name}`).join(' + ')}\nDominant: ${marketSessions.dominant}\nUTC: ${marketSessions.utcH}:00`; }
        else { reply = `P: ${Number(analysis?.rawProbAbove||0).toFixed(1)}%. ${analysis?.tradeAction}. Try: why, whale, position, session, /broadcast.`; }
        setChatLog([...currentLog, { role: 'tara', text: reply }]);
      }, 500);
    }
  };

  if (!isMounted) return <div className="min-h-screen bg-[#111312] flex items-center justify-center text-[#E8E9E4]/50 font-serif text-xl animate-pulse">Initializing Tara Terminal...</div>;
    ...
    <div className="min-h-screen bg-[#111312] text-[#E8E9E4] font-sans p-2 sm:p-3 flex flex-col selection:bg-[#E8E9E4]/20 overflow-y-auto">
      <div className="w-full max-w-[1600px] mx-auto flex flex-col h-full gap-3 min-h-0">
        ...
        <div className="flex justify-between items-center border-b border-[#E8E9E4]/10 pb-2 shrink-0">
          <div className="flex items-center justify-between w-full sm:w-auto">
            <h1 className="text-lg md:text-xl font-serif tracking-tight text-white flex items-center gap-2">
              ...
              <span className="hidden md:flex items-center gap-1 text-[10px] ml-2">{marketSessions.sessions.map((s,i)=><span key={i} className={`${s.color} opacity-80`}>{s.flag}</span>)}</span>
            </h1>
            <div className="flex sm:hidden items-center gap-2">
              ...
              <button onClick={() => setShowHelp(true)} className="p-1.5 rounded-md bg-[#111312] border border-[#E8E9E4]/10 text-[#E8E9E4]/60 hover:text-white transition-colors"><IC.Help className="w-3.5 h-3.5" /></button>
            </div>
          </div>
          ...
          <div className="hidden sm:flex text-right font-sans items-center gap-4">
            <div className="flex flex-col items-end pl-4 cursor-pointer group" onClick={() => setUseLocalTime(!useLocalTime)} title="Toggle Timezone (Local/EST)">
              <div className="text-[10px] text-[#E8E9E4]/40 uppercase tracking-widest mb-0.5 flex items-center gap-1 group-hover:text-indigo-400 transition-colors">{useLocalTime ? 'LOCAL' : 'EST'} Time <IC.Clock className="w-3 h-3" /></div>
        ...
        <div className="bg-[#181A19] p-2 sm:px-4 rounded-xl border border-[#E8E9E4]/10 shadow-md flex flex-col gap-2 shrink-0 relative overflow-hidden">
               ...
               <div className="hidden lg:flex flex-col items-start w-56">
                 {positionStatus ? (
                    <div className="w-full bg-[#111312] border border-amber-500/20 rounded-lg p-1.5 shadow-inner">
                      <div className="flex justify-between items-center w-full mb-0.5 px-1"><span className="text-[9px] text-[#E8E9E4]/40 uppercase tracking-widest">POSITION</span><span className={`text-[9px] font-bold ${positionStatus.side==='UP'?'text-emerald-400':'text-rose-400'}`}>{positionStatus.side} @ ${positionStatus.entry.toFixed(0)}</span></div>
                      <div className="flex items-center justify-between w-full px-1"><span className={`text-lg font-serif font-bold ${positionStatus.pnlPct>0?'text-emerald-400':'text-rose-400'}`}>{positionStatus.pnlPct>0?'+':''}{positionStatus.pnlPct.toFixed(1)}%</span><span className={`text-[10px] uppercase font-bold tracking-widest ${positionStatus.isStopHit?'text-rose-500 animate-pulse':'text-[#E8E9E4]/40'}`}>{positionStatus.isStopHit?'STOP HIT':'SAFE'}</span></div>
                    </div>
                 ) : (
           ...
           <div className="w-full px-2 pt-1 border-t border-[#E8E9E4]/10 mt-1 hidden sm:block">
               <div className="flex justify-between items-center text-[8px] text-[#E8E9E4]/40 uppercase tracking-widest mb-1"><span>Depth of Market (DOM)</span><span>{(orderBook.localBuy/totalDOM*100).toFixed(0)}% B / {(orderBook.localSell/totalDOM*100).toFixed(0)}% S</span></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 shrink-0 flex-none min-h-0">
               ...
               <div className="flex flex-col items-center w-full flex-1 justify-center gap-3">
                 <div className="flex flex-col items-center shrink-0 mt-2">
                   <div className="flex items-center gap-2 mb-1">
                      ...
                      {analysis.regime && (<span className="text-[7px] text-indigo-400 uppercase tracking-widest bg-indigo-500/10 border border-indigo-500/20 px-1.5 py-0.5 rounded">Regime: {analysis.regime}</span>)}
                   </div>
                  ...
                  <div className="bg-[#111312] rounded-lg border border-[#E8E9E4]/5 p-2 flex-1 overflow-y-auto custom-scrollbar">
                      {(analysis.projections || []).filter(p => p.id === activeProjectionTab).map(proj => (
                          <div key={proj.id+"-det"} className="text-[8px] text-[#E8E9E4]/70 flex flex-col h-full">
                              <div className="flex justify-between items-center mb-1 border-b border-[#E8E9E4]/10 pb-1 shrink-0"><span className="font-bold text-indigo-400">TREND PATH</span><span className="opacity-60 bg-[#181A19] px-1.5 py-0.5 rounded">CONF: {proj.conf.toFixed(0)}%</span></div>
          ...
          <div className="flex flex-col gap-3 h-full md:col-span-2 lg:col-span-1">
             <div className="bg-[#181A19] p-2 rounded-xl border border-[#E8E9E4]/10 shadow-md flex flex-col shrink-0 min-h-[120px]">
                 <div className="flex justify-between items-center mb-1.5 border-b border-[#E8E9E4]/10 pb-1"><h2 className="text-[8px] font-bold text-[#E8E9E4]/80 uppercase tracking-widest flex items-center gap-1.5"><IC.Globe className="w-3.5 h-3.5 text-blue-400" /> Live Wire</h2></div>
        ...
        <div className="w-full bg-[#181A19] p-3 sm:p-4 rounded-xl border border-[#E8E9E4]/10 shadow-lg flex flex-col flex-none lg:flex-1 min-h-[400px] sm:min-h-[350px] mt-2 relative z-10">
              ...
              <div className="flex items-center gap-2">
                  <div className="flex items-center bg-[#111312] rounded-lg border border-[#E8E9E4]/5 overflow-hidden">
                      {['1m', '3m', '5m', '15m', '30m', '1h'].map(res => (
                          <button key={res} onClick={() => setChartRes(res)} className={`px-2 py-1 text-[9px] font-bold uppercase transition-colors ${chartRes === res ? 'bg-indigo-500/20 text-indigo-400' : 'text-[#E8E9E4]/40 hover:text-[#E8E9E4]/80'}`}>{res}</button>
                      ))}
                  </div>
          ...
          <div className="flex-1 w-full h-full relative rounded-md overflow-hidden bg-[#111312]">
            <LiveChart resolution={chartRes} currentPrice={currentPrice} targetMargin={targetMargin} showCandles={showCandles} rugPullActive={showRugPullAlerts && analysis?.isRugPull} showOverlays={showOverlays} globalFlow={globalFlow} fallbackData={liveHistory} projections={analysis?.projections} prediction={analysis?.prediction} liquidations={liquidations} />
          </div>
        </div>
      </div>
      ...
      {showWhaleLog && (
        <div className="fixed top-16 right-4 z-50 w-80 bg-[#181A19] border border-purple-500/30 rounded-xl shadow-2xl overflow-hidden">
          <div className="bg-[#111312] p-3 flex justify-between items-center border-b border-[#E8E9E4]/10"><span className="text-xs font-bold uppercase tracking-widest text-purple-400">🐋 Whale Log ({'>'}$25k)</span><button onClick={() => setShowWhaleLog(false)} className="opacity-50 hover:opacity-100"><IC.X className="w-4 h-4" /></button></div>
          <div className="max-h-80 overflow-y-auto p-3 space-y-2 custom-scrollbar">
            {whaleLog.length === 0 ? (<div className="text-[10px] text-[#E8E9E4]/40 italic">Waiting for whale trades ({'>'}$25K)...</div>) : (
              whaleLog.slice(0, 30).map((w, i) => { const d = new Date(w.time); return (
