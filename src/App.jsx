...
const BASELINE_VERSION='2026.05.03-v5.6.6-past-windows-tracker';
...
function TaraCallCard({taraCall,taraScorecards,taraCallLog,windowType,timeState,analysis,className}){
  if(!taraCall)return null;
    ...
    const dispReason=snap
      ...
      :'Scanning for direction — no signal until Tara commits to UP, DOWN, or sit out.';
    ...
    if(snap){
      ...
      const _need=tc.needSamples||180;
      const _samples=tc.samples||0;
      // Show "decision in ~Xs" once Tara has at least started accumulating consistency,
      // otherwise just "scanning". Never reveal direction.
      if(_samples>=5&&_need>0){
        const _eta=Math.max(0,_need-_samples);
        countdownText=_eta===0?'committing this tick':`decision in ~${_eta}s`;
        phaseHint=`${_samples}/${_need} signal samples`;
    ...
    return(
      <div className={"mb-3 px-3 py-3 rounded-lg shrink-0 "+(className||"")} style={{background:bgClr,border:'1px solid '+borderClr,boxShadow:isLockedSnap?`inset 0 0 24px ${snap.call==='UP'?'rgba(52,211,153,0.06)':'rgba(244,114,182,0.06)'}`:'none'}}>
        ...
        {analysis&&(()=>{
          ...
          if(!snap){
            tagLabel='TARA STILL ANALYZING — engine prediction shown for your reference';
            tagColor='text-[#E8E9E4]/45';
          } else if(isSatOutSnap){
            tagLabel=genDir?`TARA OVERRODE — engine called ${genDir}, Tara wasn't convinced`:'TARA SAT OUT — engine also unsure';
            tagColor='text-amber-400/85';
          } else {
            // Tara locked UP or DOWN
            if(genDir===snap.call){tagLabel='TARA AGREED — same direction as engine';tagColor='text-emerald-400/85';}
            else if(genDir&&genDir!==snap.call){tagLabel='TARA DISAGREED — split with engine';tagColor='text-rose-400/85';}
...
function TaraMemoryStrip({taraCallLog,windowType}){
  const[open,setOpen]=React.useState(false);
...
function ProjectionsCard({analysis,mobileTab,taraCall,taraScorecards,taraCallLog,windowType,timeState}){
  ...
  return(
    <div className={'bg-[#181A19] p-3 sm:p-4 rounded-xl border border-[#E8E9E4]/10 shadow-md flex flex-col relative '+(mobileTab!=='projections'?'hidden md:flex':'')}>
      ...
      <TaraCallCard taraCall={taraCall} taraScorecards={taraScorecards} taraCallLog={taraCallLog} windowType={windowType} timeState={timeState} analysis={analysis} className="hidden md:block"/>
        ...
        <div className={baselineDrift?'mb-3 p-4 rounded-lg border-2':'mb-3 p-3 rounded-lg bg-indigo-500/10 border-2 border-indigo-500/40'} style={baselineDrift?{
          ...
          {baselineDrift?(
            <>
              ...
              <div className="font-serif text-2xl text-white mb-2 tracking-tight">Tara <span style={{color:'#E5C870'}}>5.6.6</span></div>
...
function TaraApp(){
  ...
  // V5.6.2: Block writes from stale local state until first cloud snapshot has arrived.
  //   Prevents the race where a device with stale localStorage clobbers fresh cloud data
  //   on mount. Set by the listeners below. Once true, writes flow normally.
  ...
  useEffect(()=>{
    ...
    if(_scorecardsHydratedRef.current){
      cloudWriteDebounced('scorecards/tara',taraScorecards,800);
    }
  },[taraScorecards]);
  // V5.6.2: Real-time listener replaces one-shot hydration. Pulls initial cloud state on
  //   mount AND auto-receives updates from other devices via onSnapshot. Uses MAX-merge
  //   for monotonic counters — wins/losses/sitouts only ever increase, so taking the max
  //   of local and cloud values can never lose data, even with concurrent writes from
  //   multiple devices. This is the fix for "PC and mobile show different records."
  useEffect(()=>{
    const _maxMerge=(a,b)=>({
      wins:Math.max(a?.wins||0,b?.wins||0),
      losses:Math.max(a?.losses||0,b?.losses||0),
      sitouts:Math.max(a?.sitouts||0,b?.sitouts||0),
    });
    const unsub=cloudWatch('scorecards/tara',(d)=>{
      _scorecardsHydratedRef.current=true; // hydration done — local writes can flow now
      if(!d)return;
      ...
      setTaraScorecards(prev=>{
        const next15=has15?_maxMerge(prev['15m'],d['15m']):prev['15m'];
        const next5=has5?_maxMerge(prev['5m'],d['5m']):prev['5m'];
        // Skip update if values match — avoids re-render churn.
        if(JSON.stringify(next15)===JSON.stringify(prev['15m'])&&JSON.stringify(next5)===JSON.stringify(prev['5m']))return prev;
        return{...prev,'15m':next15,'5m':next5};
      });
    });
    return unsub;
  },[]);
    ...
    const unsub=cloudWatch('memory/taraCallLog',(d)=>{
      ...
      setTaraCallLog(prev=>{
        ...
        d.entries.forEach(e=>{
          ...
          // Cloud wins if: (a) we don't have it, or (b) cloud has a result we don't,
          // or (c) cloud has a more recent resolvedAt timestamp.
          const cloudIsBetter=!existing||(!existing.result&&e.result)||(e.resolvedAt&&existing.resolvedAt&&e.resolvedAt>existing.resolvedAt);
          if(cloudIsBetter){byId.set(e.id,e);changed=true;}
        });
        if(!changed)return prev;
        return Array.from(byId.values()).sort((a,b)=>(a.id||0)-(b.id||0)).slice(-500);
      });
    });
    return unsub;
  },[]);
  ...
  useEffect(()=>{
    const unsub=cloudWatch('history/pastWindows',(d)=>{
      ...
      setPastWindows(prev=>{
        const byId=new Map(prev.map(e=>[e.id,e]));
        let changed=false;
        d.entries.forEach(e=>{if(e&&e.id&&!byId.has(e.id)){byId.set(e.id,e);changed=true;}});
        if(!changed)return prev;
        return Array.from(byId.values()).sort((a,b)=>(a.id||0)-(b.id||0)).slice(-50);
      });
    });
    return unsub;
  },[]);
  ...
  const[chatLog,setChatLog]=useState([{role:'tara',text:'Tara 5.6.6 online — Past-windows tracker added · pattern-match recent outcomes inline · history is shared across devices.'}]);
  ...
  useEffect(()=>{setIsMounted(true);
    ...
    try{
      ...
      const cleanDU=(du&&!new RegExp('V1[0-9][0-9]').test(du||''))?du:'Tara 5.6.6'; // no regex literal — esbuild safe
  ...
  const broadcastToDiscord=async(type,data)=>{
    ...
    try{
      ...
      if(type==='SIGNAL')embed={
        ...
        footer:{text:'Tara 5.6.6  |  stand-down'},
        timestamp:new Date().toISOString(),
      };
      ...
      //   the engine's general lock state.
        ...
        footer:{text:'Tara 5.6.6  |  scanning'},
        timestamp:new Date().toISOString(),
      };
        ...
        footer:{text:'Tara 5.6.6  |  sit-out'},
        timestamp:new Date().toISOString(),
      };
      ...
      // V3.2.4: Tara's Call result — fires when window resolves
        ...
        embed={
          ...
          footer:{text:'Tara 5.6.6  |  futures tape  |  not financial advice'},
          timestamp:new Date().toISOString(),
        };
      }

      const res=await fetch(discordWebhook+'?wait=true',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:discordUsername||'Tara 5.6.6',avatar_url:discordAvatar||undefined,embeds:[embed]})});
      if(res.ok){
  ...
  const editDiscordMessage=async(entry,noteText)=>{
    ...
    try{
      ...
      const updatedEmbed={
        ...
        footer:{text:`Tara 5.6.6 · edited ${new Date().toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit',hour12:true})}`},
      };
  ...
  useEffect(()=>{if(timeState.nextWindow&&timeState.nextWindow!==lastWindowRef.current){if(currentPrice!==null){if(lastWindowRef.current!==''){
        ...
        if(currentPrice!==null&&_scoringStrike>0){
          ...
          if(_outcomeDir){
            ...
            setPastWindows(prev=>[...prev,_pastEntry].slice(-50));
          }
  ...
  useEffect(()=>{
    ...
    if(!isCall){
      // Not a directional call yet; reset and wait
      if(ref.dir!==null){taraCallSampleRef.current={dir:null,count:0};}
      ...
      if(analysis?.isSystemLocked){
        ...
        setTaraCallLog(prev=>[...prev,_entry].slice(-500));
      }
      return;
    }
  ...
  if(!isMounted)return<div className={'min-h-screen bg-[#111312] flex items-center justify-center text-[#E8E9E4]/50 font-serif text-xl animate-pulse'}>Initializing Tara 5.6.6...</div>;
      ...
      <main className="flex-1 w-full max-w-[1600px] mx-auto px-2 sm:px-3 lg:px-4 py-2 sm:py-3 flex flex-col gap-3 min-h-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.25fr_1fr_1fr] gap-3 shrink-0 auto-rows-fr">
          ...
          <div className={`bg-[#181A19] p-3 sm:p-4 rounded-xl border border-[#E8E9E4]/10 shadow-md flex flex-col relative ${mobileTab!=='signal'?'hidden md:flex':''}`}>
            ...
            <TaraCallCard taraCall={taraCall} taraScorecards={taraScorecards} taraCallLog={taraCallLog} windowType={windowType} timeState={timeState} analysis={analysis} className="md:hidden"/>
          ...
          <ProjectionsCard analysis={analysis} mobileTab={mobileTab} taraCall={taraCall} taraScorecards={taraScorecards} taraCallLog={taraCallLog} windowType={windowType} timeState={timeState}/>
      ...
      <div className={`fixed bottom-4 right-4 z-50 flex flex-col items-end transition-all ${isChatOpen?'w-[90vw] sm:w-80':'w-auto'}`}>
        {isChatOpen&&(
          <div className={'bg-[#181A19] border border-[#E8E9E4]/20 shadow-2xl rounded-xl w-full mb-3 overflow-hidden flex flex-col h-[55vh] sm:h-96'}>
            <div className={'bg-[#111312] p-2.5 flex justify-between items-center border-b border-[#E8E9E4]/10'}><span className="text-xs font-bold uppercase tracking-wide flex items-center gap-2"><IC.Msg className="w-3.5 h-3.5 text-indigo-400"/>Chat with Tara 5.6.6</span><button onClick={()=>setIsChatOpen(false)} className="opacity-50 hover:opacity-100"><IC.X className="w-4 h-4"/></button></div>
            ...
            <div className={'sticky top-0 bg-[#181A19] border-b border-[#E8E9E4]/10 p-4 flex justify-between items-center z-10'}>
              <div>
                <h2 className="text-base sm:text-lg font-serif text-white flex items-center gap-2">
                  <span className="text-indigo-400 text-xl font-bold">?</span> How Tara 5.6.6 Works
                </h2>
        ...
        <div className={'fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4'}>
          <div className={'bg-[#181A19] border border-[#E8E9E4]/20 rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto shadow-2xl'} style={{scrollbarWidth:'thin'}}>
            <div className={'sticky top-0 bg-[#181A19] border-b border-[#E8E9E4]/10 p-4 flex justify-between items-center'}>
              <h2 className="text-base sm:text-lg font-serif text-white flex items-center gap-2"><IC.Info className="w-5 h-5 text-indigo-400"/>Tara 5.6.6 — What's New</h2>
